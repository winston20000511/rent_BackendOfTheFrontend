...
CREATE TABLE Messages (
    id bigint PRIMARY KEY IDENTITY(1,1), --modify
...
DECLARE @targetConversationCount INT = 20; -- 目標對話數
DECLARE @targetUserCount INT = 10; -- 目標參與人數

-- 52 (官方帳號) 對所有人發送歡迎訊息
WHILE @counter <= @maxUserId
BEGIN
    IF @counter != 52
    BEGIN
        INSERT INTO Messages (sender_id, receiver_id, message, timestamp)
...
BEGIN
    SET @randValue = ABS(CHECKSUM(NEWID())) % (@maxUserId - @minUserId + 1) + @minUserId;
    IF @randValue NOT IN (52) AND NOT EXISTS (SELECT 1 FROM @selectedUsers WHERE userId = @randValue)
    BEGIN
...
BEGIN
    ...
    SELECT TOP 1 @currentUserId = userId FROM @selectedUsers;

    -- 計算還需要多少對話
    DECLARE @remainingConversations INT = @targetConversationCount - (SELECT COUNT(*) FROM Messages WHERE sender_id = @currentUserId OR receiver_id = @currentUserId);

    WHILE @remainingConversations > 0
    BEGIN
        SET @randValue = ABS(CHECKSUM(NEWID())) % (@maxUserId - @minUserId + 1) + @minUserId;

        -- 避免與自己對話，且避免與 52 號官方帳號對話
        IF @randValue NOT IN (@currentUserId, 52) AND EXISTS (SELECT 1 FROM @selectedUsers WHERE userId = @randValue)
        BEGIN
            INSERT INTO Messages (sender_id, receiver_id, message, timestamp)
            VALUES (
                @currentUserId,
                @randValue,
                '這是 ' + CAST(@currentUserId AS VARCHAR) + ' 發送給 ' + CAST(@randValue AS VARCHAR) + ' 的訊息',
                GETDATE()
            );

            SET @remainingConversations = @remainingConversations - 1;
        END
    END;

    DELETE FROM @selectedUsers WHERE userId = @currentUserId;
END;
...
CREATE TABLE complaints (
    complaints_id bigint PRIMARY KEY IDENTITY(1,1), --modify origin int
	user_id bigint NOT NULL, --modify origin int
    username NVARCHAR(20) NOT NULL,     
    category NVARCHAR(10) NOT NULL,     
    subject NVARCHAR(50) NOT NULL,      
    content NVARCHAR(100) NOT NULL,
	note NVARCHAR(100) DEFAULT '無',
	status NVARCHAR(10)DEFAULT '待處裡',--modify not default
...
CREATE TABLE photos (
    ...
    complaints_id bigint, --modify origin int
