...
public interface MessageRepository extends JpaRepository<MessageBean, Long> {
	...
	@Query("SELECT m.id AS messageId, m.message AS message, m.timestamp AS timestamp, " +
		       "s.userId AS senderId, s.name AS senderName, " +
		       "r.userId AS receiverId, r.name AS receiverName, " +
		       ...
		       "(s.userId = :targetUserId AND r.userId = :currentUserId) " +
		       "ORDER BY m.timestamp ASC")
		List<MessageWithUserDTO> findMessagesWithUserDetails(
	...
	@Query("SELECT m FROM MessageBean m WHERE " +
		      ...
		      "(m.sender.userId = :targetUserId AND m.receiver.userId = :currentUserId) " +
		      "ORDER BY m.timestamp ASC")
	...
	@Query("SELECT m FROM MessageBean m WHERE " +
		...
		List<MessageBean> findMessagesBetweenUsersAfter(
		    @Param("currentUserId") Long currentUserId,
		    @Param("targetUserId") Long targetUserId,
	...
	List<MessageBean> findBySenderIdAndReceiverIdOrSenderIdAndReceiverId(
